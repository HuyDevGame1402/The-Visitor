	...
	public class SwfManager : MonoBehaviour
	{
		...
		public static SwfManager GetInstance(bool allow_create)
		{
			if (!_instance)
			{
				...
				{
					GameObject gameObject = new GameObject("[SwfManager]");
					_instance = gameObject.AddComponent<SwfManager>();
				}
			}
			return _instance;
		}
		...
		{
			float value;
			return (!_groupRateScales.TryGetValue(group_name, out value)) ? 1f : value;
		}
			...
			for (int count = _safeUpdates.Count; i < count; i++)
			{
				...
				if ((bool)swfClipController)
				{
					...
					else if (IsGroupPlaying(groupName))
					{
						...
						swfClipController.Internal_Update(groupRateScale * ((!IsGroupUseUnscaledDt(groupName)) ? scaled_dt : unscaled_dt), groupRateScale * unscaled_dt);
					}
				}
			}
			_safeUpdates.Clear();
		...
		private void LateUpdate()
		{
			if (isPlaying)
			{
				LateUpdateControllers(rateScale * ((!useUnscaledDt) ? Time.deltaTime : Time.unscaledDeltaTime), rateScale * Time.unscaledDeltaTime);
			}
			LateUpdateClips();
		}
	}
